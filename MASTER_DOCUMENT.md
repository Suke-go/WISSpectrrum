...
### 3.8 計算処理フローの詳細
- テキスト抽出後、要約対象の本文をチャンク化し（例: 2,500 文字 + オーバーラップ 250 文字）、各チャンクを LLM で要約・再構成する。
- セクションごとの要約生成ではプロンプトテンプレートと検証ルール（語数上限、欠落チェック）を適用し、失敗時は再試行ロジックを実装する。
- 埋め込み計算は Gemini Embedding Model にバッチ入力し、正規化済みベクトルを得て保存する。類似度は基本的に `cosine(u, v) = (u・v) / (||u|| ||v||)` を採用し、全体ベクトルとセクション別ベクトルの両方で計算する。
- ACM CCS 分類は 2 段階で行う: (1) Sentence Transformer などで候補概念を上位 N 件抽出、(2) LLM に候補と要約を渡して最適な概念を 1〜3 件選定させる。
- 類似度検索はベクトルデータベース（例: Faiss、Milvus、Pinecone など）またはローカルの JSONL 索引を利用し、クエリごとにトップ K 結果と距離値を返す API を整備する。
- 参考: 将来的に差別化したい場合は、クラスタリング（HDBSCAN など）、多次元尺度法 (MDS)、トピックモデリングといった補助的な演算を追加し、視覚化のバリエーションを増やす。
